...
class ModelLocalisationOrderStatus extends Model {
	public function addOrderStatus($data) {
		foreach ($data['order_status'] as $language_id => $value) {
			if (isset($order_status_id)) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_status SET order_status_id = '" . (int)$order_status_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "',specified = '".(int)$data['order_status_specified']."'");
			} else {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_status SET language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "',specified = '".(int)$value['specified']."'");
	...
	public function getOrderStatuses($data = array()) {
      	if ($data) {
			...
			if(isset($data['specified'])){
				$sql .= " AND specified = '".(int)$data['specified']."'";
			}
	...
	public function getOrderSpecifiedStatuses($data=array()){
			$sql = "SELECT * FROM " . DB_PREFIX . "order_status WHERE specified = 1 AND language_id = '" . (int)$this->config->get('config_language_id') . "'";
